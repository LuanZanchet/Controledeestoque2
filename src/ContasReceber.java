...
public class ContasReceber extends JFrame{
	
	
	private static final long serialVersionUID = 1L;
	private JLabel jlbCliente = new JLabel("Cliente"),jlbValor = new JLabel("Valor:"); 
	...
	private File receber=new File("ContasReceber.txt");
	...
	ContasReceber() {
		...
		setLayout(null);
		posicionaObjeto(jlbCliente, 10, 10, 100, 25);
		...
		FileReader fr = null;
		try {
			fr = new FileReader("ContasReceber.txt");
		...
		jbtCadastrar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(atualiza==0){
					JOptionPane.showInputDialog(this, "Atualize antes de Cadastrar!");
				}
				else{
				...
				try {
					FileWriter fw= new FileWriter("ContasReceber.txt",true);
					BufferedWriter bw= new BufferedWriter(fw);
					boolean existente=receber.exists();
					if(existente){
		...
		jbtLimpar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Writer out;
				try {
					out = new FileWriter("ContasReceber.txt");
					out.write("");
					out.flush();    
					out.close();  
					FileWriter fe= new FileWriter("ContasReceber.txt",true);
					BufferedWriter be= new BufferedWriter(fe);
						boolean existe=receber.exists();
						if(existe){
